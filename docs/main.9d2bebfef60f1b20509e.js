...
var divTodoList=document.querySelector(".todo-list"),txtInput=document.querySelector(".new-todo"),btnBorrar=document.querySelector(".clear-completed"),ulFiltros=document.querySelector(".filters"),anchorFiltros=document.querySelectorAll(".filtro"),contador=document.getElementsByTagName("strong")[0];var crearTodoHTML=function(a){var b="\n    <li class=\"".concat(a.completado?"completed":"","\" data-id=\"").concat(a.id,"\">\n        <div class=\"view\">\n            <input class=\"toggle\" type=\"checkbox\" ").concat(a.completado?"checked":"",">\n            <label>").concat(a.tarea,"</label>\n            <button class=\"destroy\"></button>\n        </div>\n        <input class=\"edit\" value=\"Create a TodoMVC template\">\n    </li>\n    "),c=document.createElement("div");return c.innerHTML=b,divTodoList.append(c.firstElementChild),c.firstElementChild};//Eventos
txtInput.addEventListener("keyup",function(a){if(13===a.keyCode&&0<txtInput.value.length){var b=new Todo(txtInput.value);todoList.nuevoTodo(b),crearTodoHTML(b),txtInput.value="",actualizadorDeConteo()}}),divTodoList.addEventListener("click",function(a){var b=a.target.localName,c=a.target.parentElement.parentElement,d=c.getAttribute("data-id");b.includes("input")?(todoList.marcarCompletado(d),c.classList.toggle("completed"),actualizadorDeConteo()):b.includes("button")&&(todoList.eliminarTodo(d),divTodoList.removeChild(c),actualizadorDeConteo())}),btnBorrar.addEventListener("click",function(){todoList.eliminarCompletados(),actualizadorDeConteo();for(var a,b=divTodoList.children.length-1;0<=b;b--)a=divTodoList.children[b],a.classList.contains("completed")&&divTodoList.removeChild(a)}),ulFiltros.addEventListener("click",function(a){var b=a.target.text;if(b){anchorFiltros.forEach(function(a){return a.classList.remove("selected")}),a.target.classList.add("selected");var c,d=_createForOfIteratorHelper(divTodoList.children);try{for(d.s();!(c=d.n()).done;){var e=c.value;e.classList.remove("hidden");var f=e.classList.contains("completed");"Pendientes"===b?f&&e.classList.add("hidden"):"Completados"===b?f||e.classList.add("hidden"):void 0}}catch(a){d.e(a)}finally{d.f()}}});// Funciones
var actualizadorDeConteo=function(){var a,b=todoList.todos.length,c=_createForOfIteratorHelper(todoList.todos);try{for(c.s();!(a=c.n()).done;){var d=a.value;d.completado&&b--}}catch(a){c.e(a)}finally{c.f()}contador.innerHTML=b};
...
var todoList=new TodoList;todoList.todos.forEach(/*todo => */crearTodoHTML/*(todo)*/),actualizadorDeConteo(),console.log("prueba");
/******/ })()
;